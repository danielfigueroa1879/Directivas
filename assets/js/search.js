...
class GlobalSearch {
    ...
    displayResults(results, query) {
        ...
        if (results.length === 0) {
            ...
            // ✅ SEGURO: Función helper para sanitizar HTML
            const sanitizeHTML = (html) => {
                const div = document.createElement('div');
                div.textContent = html; // textContent escapa automáticamente HTML
            ...
            resultsContainer.innerHTML = results.map((item, index) => {
                ...
                // ✅ Sanitizar datos antes de insertarlos en HTML
                const safeSection = sanitizeHTML(item.section);
                const safeTitle = sanitizeHTML(highlightedTitle);
                const safeContent = sanitizeHTML(highlightedContent);

                return `
                    <div class="search-result-item" data-index="${index}">
                        <div class="result-section-badge">${safeSection}</div>
                        <div class="result-content">
                            <h4 class="result-title">${safeTitle}</h4>
                            <p class="result-description">${safeContent}</p>
                        </div>
