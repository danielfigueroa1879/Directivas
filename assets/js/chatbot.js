...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ✅ SEGURO: API key ahora está en el servidor (variable de entorno de Netlify)
    // NO guardar claves en el código cliente - usar proxy backend en lugar de eso 
...
async function speakWithElevenLabs(text) {
        try {
            // ✅ SEGURO: Usar proxy backend en lugar de llamar directamente a ElevenLabs
            const response = await fetch('/.netlify/functions/elevenlabs-proxy', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                console.error('Error en síntesis de voz:', errorData);
                throw new Error(`Error en síntesis: ${response.status}`);
            }

            // ✅ Respuesta del proxy es audio en base64
            const responseData = await response.json();
            const audioBlob = new Blob(
                [Uint8Array.from(atob(responseData), c => c.charCodeAt(0))],
                { type: 'audio/mpeg' }
            );
...
// Función principal de TTS con fallback inteligente
    async function speakText(text) {
       ...
       const cleanText = text
        ...
        try {
            // ✅ SEGURO: Intentar ElevenLabs a través del proxy backend
            ...
            // Fallback: si ElevenLabs falla, usar voz del navegador
    ...
    function changeVoice(voiceID) {
        if (voiceID && Object.values(ELEVENLABS_VOICES).includes(voiceID)) {
            ...
            // ✅ SEGURO: Siempre intentar con ElevenLabs a través del proxy
